...
public class TSIG {
    ...
    public TSIGRecord generate(Message m, byte[] b, int error, TSIGRecord old) {
        ...
        out.writeU16(KEYRecord.PROTOCOL_ANY);
        out.writeU32(0);
        ...
        return new TSIGRecord(this.name, KEYRecord.PROTOCOL_ANY, 0, this.alg, timeSigned, fudge, signature, m.getHeader().getID(), error, other);
    }
    ...
    public void applyStream(Message m, TSIGRecord old, boolean first) {
        ...
        hmac.update(out.toByteArray());
        Message message = m;
        message.addRecord(new TSIGRecord(this.name, KEYRecord.PROTOCOL_ANY, 0, this.alg, timeSigned, fudge, hmac.sign(), m.getHeader().getID(), 0, null), 3);
        m.tsigState = 3;
    }
