...
abstract class KEYBase extends Record {
    ...
    public int getFootprint() {
        ...
        int d2;
        if (this.alg == 1) {
            d2 = rdata[rdata.length - 2] & KEYRecord.PROTOCOL_ANY;
            foot = ((rdata[rdata.length - 3] & KEYRecord.PROTOCOL_ANY) << 8) + d2;
        } else {
            int i = 0;
            while (i < rdata.length - 1) {
                d2 = rdata[i + 1] & KEYRecord.PROTOCOL_ANY;
                foot += ((rdata[i] & KEYRecord.PROTOCOL_ANY) << 8) + d2;
                i += 2;
            }
            if (i < rdata.length) {
                foot += (rdata[i] & KEYRecord.PROTOCOL_ANY) << 8;
            }
