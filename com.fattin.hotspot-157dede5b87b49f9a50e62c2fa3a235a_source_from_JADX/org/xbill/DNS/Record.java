...
public abstract class Record implements Cloneable, Comparable, Serializable {
    ...
    public String toString() {
        ...
        if (!rdata.equals(ACRAConstants.DEFAULT_STRING_VALUE)) {
            sb.append("\t");
            sb.append(rdata);
        }
        return sb.toString();
    ...
    protected static byte[] byteArrayFromString(String s) throws TextParseException {
        ...
        if (hasEscapes) {
            ...
            for (i = 0; i < array.length; i++) {
                ...
                if (escaped) {
                    if (b2 >= 48 && b2 <= 57 && digits < 3) {
                        ...
                        if (intval > KEYRecord.PROTOCOL_ANY) {
            ...
            if (digits > 0 && digits < 3) {
                throw new TextParseException("bad escape");
            } else if (os.toByteArray().length <= KEYRecord.PROTOCOL_ANY) {
                return os.toByteArray();
            } else {
                throw new TextParseException("text string too long");
            }
        } else if (array.length <= KEYRecord.PROTOCOL_ANY) {
            return array;
        } else {
            throw new TextParseException("text string too long");
        }
    ...
    protected static String byteArrayToString(byte[] array, boolean quote) {
        ...
        for (byte b : array) {
            int b2 = b & KEYRecord.PROTOCOL_ANY;
    ...
    public int hashCode() {
        ...
        for (byte b : toWireCanonical(true)) {
            code += (code << 3) + (b & KEYRecord.PROTOCOL_ANY);
        }
        return code;
    }
    ...
    public int compareTo(Object o) {
        ...
        while (i < rdata1.length && i < rdata2.length) {
            n = (rdata1[i] & KEYRecord.PROTOCOL_ANY) - (rdata2[i] & KEYRecord.PROTOCOL_ANY);
            if (n != 0) {
                return n;
            }
            i++;
    ...
    static int checkU8(String field, int val) {
        if (val >= 0 && val <= KEYRecord.PROTOCOL_ANY) {
            return val;
        }
