        ...
        for (int i = 0; i < maxHits; i++) {
            ...
            if (url == null) {
                if (Log.isDebugEnabled()) {
                    Log.m10w("No destination: discarding hit: " + hit.getHitParams());
                } else {
                    Log.m10w("No destination: discarding hit.");
                }
                hitsDispatched++;
            } else {
                HttpHost targetHost = new HttpHost(url.getHost(), url.getPort(), url.getProtocol());
                String path = url.getPath();
                String params = TextUtils.isEmpty(hit.getHitParams()) ? ACRAConstants.DEFAULT_STRING_VALUE : HitBuilder.postProcessHit(hit, System.currentTimeMillis());
                ...
                if (request == null) {
                    ...
                    if (params.length() > Flags.FLAG2) {
                        Log.m10w("Hit too long (> 8192 bytes)--not sent");
                    } else {
                        try {
                            HttpResponse response = client.execute(targetHost, request);
                            if (response.getStatusLine().getStatusCode() != RestResponse.RESULT_CODE_EMAIL_ALREADY_IN_USE) {
                                Log.m10w("Bad response: " + response.getStatusLine().getStatusCode());
                                break;
                            }
                        } catch (ClientProtocolException e) {
                            Log.m10w("ClientProtocolException sending hit; discarding hit...");
                        } catch (IOException e2) {
                            Log.m10w("Exception sending hit: " + e2.getClass().getSimpleName());
                            Log.m10w(e2.getMessage());
                        }
                    }
                    hitsDispatched++;
                }
    ...
    private HttpEntityEnclosingRequest buildRequest(String params, String path) {
        ...
        if (full.length() < 2036) {
            ...
            try {
                ...
                Log.m10w("Encoding error, discarding hit");
                return null;
            }
        }
        ...
        if (debug) {
            ...
            if (request.getEntity() != null) {
                try {
                    ...
                    Log.m10w("Error Writing hit to log...");
                }
            }
            Log.m8i(httpHeaders.toString());
        }
    }
